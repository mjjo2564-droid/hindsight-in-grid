...
def compute_reward(state, goal, x_size, y_size):
    distance = abs(state[0] - goal[0]) + abs(state[1] - goal[1])
    max_distance = (x_size - 1) + (y_size - 1)
...
def add_her_experiences(replay, trajectory, x_size, y_size):
    for index, transition in enumerate(trajectory):
        ...
        for _ in range(HER_K):
            future_index = random.randint(index, len(trajectory) - 1)
            new_goal = trajectory[future_index][3]
            new_done = next_state == new_goal
            new_reward = compute_reward(next_state, new_goal, x_size, y_size)

            replay.append((state, action, new_reward, next_state, new_goal, new_done))


def save_graph(history):
    output_dir = "results/256"
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "dqn_her.png"))
    plt.close()
...
def train():
    ...
    for episode in range(1, EPISODES + 1):
        ...
        for step in range(1, MAX_STEPS + 1):
            ...
            reward = compute_reward(next_state, goal, x_size, y_size)

            transition = (state, action, reward, next_state, goal, success)
        ...
        add_her_experiences(replay, trajectory, x_size, y_size)
